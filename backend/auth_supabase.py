...
class UserManager:
    ...
    @staticmethod
    def save_deck(user_id: int, name: str, commander_name: str, deck_data: dict, deck_analysis: dict = None, is_public: bool = False):
        """Save a generated deck to the database"""
        deck_id = db.execute_query_one('''
            INSERT INTO saved_decks (user_id, name, commander_name, deck_data, deck_analysis, is_public)
            VALUES (%s, %s, %s, %s, %s, %s) RETURNING id
        ''', (user_id, name, commander_name, json.dumps(deck_data), 
              json.dumps(deck_analysis) if deck_analysis else None, is_public))
    ...
    def get_user_decks(user_id: int):
        ...
        return db.execute_query('''
            SELECT id, name, commander_name, is_public, created_at, updated_at
